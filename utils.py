...
def is_nsfw(text) -> bool:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Is the given text NSFW? If yes, return `1``, else return `0`.",
            },
...
def is_not_question(text) -> bool:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Is the given text a question? If yes, return `1``, else return `0`.",
            },
            {"role": "user", "content": text},
        ],
        max_tokens=1,
        ...
        if delta.type == "code_interpreter" and delta.code_interpreter:
            ...
            if delta.code_interpreter.input:
                ...
                with st.session_state.text_boxes[-1]:
                    ...
                    if (
                        f"code_box_{len(st.session_state.text_boxes)}"
                        not in st.session_state
                    ):
                        # Nest the code in an expander
                        st.session_state[
                            f"code_expander_{len(st.session_state.text_boxes)}"
                        ] = st.status("**💻 Code**", expanded=True)
